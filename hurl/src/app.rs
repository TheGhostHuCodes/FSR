...
pub struct App {
    ...
    ///
    /// A token which will be sent as "Bearer <token>" in the authorization
    /// header.
    #[structopt(short, long)]
    ...
    /// Default transport.
    ///
    /// If a URL is given without a transport, i.e. example.com/foo, http will be
    /// used as the transport by default. If this flag is set then https will be
    /// used instead.
    #[structopt(short, long)]
    pub secure: bool,
    ...
    /// The URL to issue a request if a method subcommand is not specified.
    ...
    ///
    ///     e.g. foo=@bar.txt becomes {"foo": "the contents of bar.txt"} or form
    ///     encoded
    ///
    /// Raw JSON data where the value should be parsed to JSON first --
    /// key:=value
    ///
